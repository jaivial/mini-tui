...
/** Case-insensitive substring filter over provider names (and ids). */
export function filterProviders<T extends { id: string; name: string }>(providers: T[], query: string): T[] {
  const needle = query.trim().toLowerCase();
